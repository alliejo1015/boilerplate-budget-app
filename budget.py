class Category:
  def __init__(self, description):
    ...
    self.ledger = [] #create empty placeholder
    self.balance = 0.00  #initialize balance at $0
  ...
  def __repr__(self):
    ...
    ledger = ""
      for item in self.ledger:
        ...
        #ledger description set to 23 and 7 characters
  ...
  def transfer(self, amount, category):
    if self.withdraw(amount, "Transfer to {}".format(category.description)):
      category.deposit(amount, "Transfer from {}".format(category.description))
      return True
    else:
      return False
...
def create_spend_chart(categories):
  ...
  #calculate percentage spent on each category
  total = round(sum(total_spent),2)
  percent = list(map(lambda amount: int((((amount / total) * 10) // 1) * 10), total_spent))
